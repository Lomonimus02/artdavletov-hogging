...
const Navigation = ({ projectsActiveSection = null }: NavigationProps) => {
  ...
  // - About page = always dark elements (black hamburger on white background)
  // - Other cases = based on isDarkBackground
  const shouldUseWhiteElements = !isMenuOpen && (isInHeroSection || (pathname === '/' && isDarkBackground) || (pathname !== '/' && pathname !== '/projects' && pathname !== '/about' && isDarkBackground));
  ...
  const detectBackgroundColor = () => {
    ...
    // Special case: About page should always have dark navigation
    if (pathname === '/about') {
      console.log('About page - forcing dark menu elements');
  ...
  useEffect(() => {
    ...
    if (pathname === '/') {
      ...
      // For about page, always set light background (dark navigation)
      console.log('Pathname change: Setting dark menu for about page');
      setIsDarkBackground(false);
          ...
          <div className="flex justify-between items-center h-16">
            ...
            <div className={`flex-shrink-0 transition-opacity duration-300 ${shouldHideLogo ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}>
              <Link href="/" className="mobile-logo-container">
                {/* Desktop text logo */}
                <span className={`text-lg font-bold tracking-wider uppercase mobile-hide-text-logo transition-colors duration-300 ${
      ...
      >
        <div className="h-full flex flex-col justify-center items-center px-6 md:px-12">
          <div className="space-y-8 md:space-y-12 text-center">
            {menuItems.map((item, index) => (
              <Link
                key={item.href}
                href={item.href}
                className={`block text-black hover:text-gray-600 text-xl md:text-3xl font-bold uppercase tracking-wider transition-all duration-500 transform hover:scale-105 ${
          ...
          <div className="mt-16 pt-8 border-t border-gray-200 w-full max-w-md">
            <h3 className="text-sm md:text-base font-bold uppercase tracking-wider text-gray-500 mb-4 text-center">
              {t('nav.language')}
            </h3>
            <div className="space-y-4 md:space-y-6 flex flex-col items-center">
              <button
                className={`inline-block text-lg md:text-xl font-medium uppercase tracking-wider transition-all duration-500 transform hover:scale-105 ${
