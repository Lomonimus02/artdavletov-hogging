...
export default function Projects() {
        ...
        if (response.ok) {
          ...
          if (data.projects && data.cities) {
            ...
            const cities = Array.from(new Set(
              data.map((project: any) => project.city).filter((city: string) => city && city.trim() !== '')
            )).sort();
  ...
  const projectTypes = [
    ...
    { key: 'architectural', label: t('projects.filter.architectural') }
  ];
  ...
  // Создаем динамические фильтры городов на основе доступных городов с кастомным порядком
  const priorityCities = ['Англия', 'Польша', 'Чешская республика'];
  const otherCities = availableCities.filter(city => !priorityCities.includes(city)).sort();
  const orderedCities = [...priorityCities.filter(city => availableCities.includes(city)), ...otherCities];
  ...
  const filteredProjects = projects.filter(project => {
    // Type matching with proper translation mapping
    let typeMatch = true;
    if (selectedType && selectedType !== t('projects.filter.all') && selectedType !== 'Все' && selectedType !== '') {
      if (selectedType === t('projects.filter.design')) {
        typeMatch = project.type === 'Дизайн проект' || project.type === 'Дизайн-проект';
      } else if (selectedType === t('projects.filter.sketch')) {
        typeMatch = project.type === 'Эскизный проект';
      } else if (selectedType === t('projects.filter.architectural')) {
        typeMatch = project.type === 'Архитектурный проект';
      } else {
        typeMatch = project.type === selectedType;
      }
    }

    // City matching - check both original city name and translated version
    let cityMatch = true;
    if (selectedCity && selectedCity !== t('projects.filter.all') && selectedCity !== 'Все' && selectedCity !== '') {
      const originalCity = project.city;
      const translatedCity = translateCity(originalCity);
      cityMatch = originalCity === selectedCity || translatedCity === selectedCity;
    }
  ...
  const handleSectionNavigation = (sectionId: string) => {
    ...
    // Update active section for logo display
  ...
  // Handle section navigation from hash in URL
  useEffect(() => {
    const handleHashNavigation = () => {
      const hash = window.location.hash.replace('#', '');
      if (hash && ['architecture', 'urbanism', 'interior'].includes(hash)) {
        console.log(`[SECTION NAVIGATION] Starting scroll to section: ${hash}`);
        console.log(`[SECTION NAVIGATION] Window width: ${window.innerWidth}, is mobile: ${window.innerWidth <= 768}`);

        // Wait for page to be fully loaded and rendered
        const scrollToSection = (attempts = 0, maxAttempts = 40) => {
          // Wait for projects to load first
          if (loadingProjects && attempts < maxAttempts) {
            console.log(`[SECTION NAVIGATION] Still loading projects, waiting... (attempt ${attempts + 1}/${maxAttempts})`);
            setTimeout(() => scrollToSection(attempts + 1, maxAttempts), 150);
            return;
          }

          // Try to find the element - prioritize visible elements based on screen size
          const isMobile = window.innerWidth <= 768;
          let element = null;

          if (isMobile) {
            // On mobile, look for elements in the mobile layout first
            element = document.querySelector(`.mobile-projects-grid [data-service-id="${hash}"]`);
            if (!element) {
              // Fallback to any element with the data-service-id
              element = document.querySelector(`[data-service-id="${hash}"]`);
            }
          } else {
            // On desktop, look for elements in the desktop layout first
            element = document.querySelector(`.hidden.md\\:block [data-service-id="${hash}"]`);
            if (!element) {
              // Fallback to any element with the data-service-id
              element = document.querySelector(`[data-service-id="${hash}"]`);
            }
          ...
          console.log(`[SECTION NAVIGATION] Looking for element with data-service-id="${hash}" (attempt ${attempts + 1}/${maxAttempts})`);
          console.log(`[SECTION NAVIGATION] Mobile mode: ${isMobile}, Found element:`, element);

          if (element) {
            console.log(`[SECTION NAVIGATION] Found element for ${hash}!`);
            ...
            setTimeout(() => {
              ...
              console.log(`[SECTION NAVIGATION] Element visible: ${isVisible}, display: ${computedStyle.display}`);

              if (!isVisible && attempts < maxAttempts - 5) {
                console.log(`[SECTION NAVIGATION] Element not visible yet, retrying...`);
                setTimeout(() => scrollToSection(attempts + 1, maxAttempts), 150);
                return;
              }

              const elementPosition = element.getBoundingClientRect().top + window.pageYOffset;
              const offsetPosition = isDesktop ? elementPosition - 80 : elementPosition - 100;

              console.log(`[SECTION NAVIGATION] Scrolling to position: ${offsetPosition} (desktop: ${isDesktop})`);
              console.log(`[SECTION NAVIGATION] Element rect:`, element.getBoundingClientRect());
              ...
              // Clear hash after successful scroll to prevent re-triggering
              setTimeout(() => {
            ...
            console.log(`[SECTION NAVIGATION] Element not found for ${hash}, retrying...`);
            // Log all available elements for debugging
            const allElements = document.querySelectorAll('[data-service-id]');
            const mobileElements = document.querySelectorAll('.mobile-projects-grid [data-service-id]');
            const desktopElements = document.querySelectorAll('.hidden.md\\:block [data-service-id]');

            console.log(`[SECTION NAVIGATION] All elements:`, Array.from(allElements).map(el => el.getAttribute('data-service-id')));
            console.log(`[SECTION NAVIGATION] Mobile elements:`, Array.from(mobileElements).map(el => el.getAttribute('data-service-id')));
            console.log(`[SECTION NAVIGATION] Desktop elements:`, Array.from(desktopElements).map(el => el.getAttribute('data-service-id')));

            // Try again after a delay
            setTimeout(() => scrollToSection(attempts + 1, maxAttempts), 150);
          } else {
            console.log(`[SECTION NAVIGATION] Max attempts reached, giving up on ${hash}`);
          }
        };

        // Start scrolling process - different delays for mobile vs desktop
        const isMobile = window.innerWidth <= 768;
        const initialDelay = isMobile ? 1500 : 1000;
        setTimeout(scrollToSection, initialDelay);
      }
    };

    // Handle initial hash when component mounts
  ...
  }, [loadingProjects]); // Only depend on loading state

  // Additional effect to handle hash navigation after projects are fully loaded
  useEffect(() => {
    // Only run this effect when projects finish loading and we have projects
    if (!loadingProjects && filteredProjects.length > 0) {
      const hash = window.location.hash.replace('#', '');
      if (hash && ['architecture', 'urbanism', 'interior'].includes(hash)) {
        console.log(`[SECTION NAVIGATION BACKUP] Projects loaded, attempting backup navigation to: ${hash}`);

        // Give extra time for DOM to settle, especially important on mobile
        setTimeout(() => {
          const isMobile = window.innerWidth <= 768;
          let element = null;

          // Try to find the element with the same logic as main handler
          if (isMobile) {
            element = document.querySelector(`.mobile-projects-grid [data-service-id="${hash}"]`);
            if (!element) {
              element = document.querySelector(`[data-service-id="${hash}"]`);
          ...
          if (element) {
            console.log(`[SECTION NAVIGATION BACKUP] Found element, scrolling...`);
            const isDesktop = window.innerWidth > 768;
            const elementPosition = element.getBoundingClientRect().top + window.pageYOffset;
            const offsetPosition = isDesktop ? elementPosition - 80 : elementPosition - 100;
            ...
            // Clear hash after successful scroll
            setTimeout(() => {
              if (window.location.hash) {
                history.replaceState(null, '', window.location.pathname);
              }
            ...
            console.log(`[SECTION NAVIGATION BACKUP] Element still not found for ${hash}`);
          }
        }, 800);
      }
    }
  ...
  return (
    <>
      {/* Custom Navigation for Projects Page */}
      <ProjectsNavigation activeSection={activeSection} />

      <div className="min-h-screen bg-white pt-16">
        {/* Section Logo - only show when user is in a specific section */}
        {activeSection && activeSection !== 'header' && (
          <SectionLogo activeSection={activeSection} isDarkBackground={false} />
        )}

        {/* Section Navigation */}
        <ProjectsSectionNavigation
          onNavigate={handleSectionNavigation}
          onSectionChange={setActiveSection}
        />

      {/* Header */}
      <section className="py-16 px-4 sm:px-6 lg:px-8 border-b border-gray-100 projects-page-header" data-service-id="header">
        <div className="max-w-7xl mx-auto">
          <div className="text-center">
            <h1 className="text-8xl md:text-6xl lg:text-7xl xl:text-8xl font-bold text-black tracking-wider uppercase leading-tight">
              {t('projects.title')}
            </h1>
            {isAdmin && (
              <div className="mt-8">
                <Link
                  ...
                  className="bg-blue-600 text-white px-6 py-3 font-bold uppercase tracking-wider text-sm hover:bg-blue-700 transition-colors duration-300"
                >
          ...
          <div className="flex items-center justify-center gap-4 mt-8">
            <a
              href="/contacts"
              className="inline-flex items-center justify-center bg-white text-black border-2 border-black px-8 py-3 font-bold uppercase tracking-wider text-lg md:text-xl lg:text-2xl hover:bg-black hover:text-white transition-colors duration-500 h-[52px]"
            >
            ...
            <button
              ...
              className="bg-white text-black border-2 border-black px-4 py-3 font-bold uppercase tracking-wider hover:bg-black hover:text-white transition-colors duration-500 flex items-center justify-center h-[52px]"
      ...
      {/* Filter Section */}
      <div className={`overflow-hidden transition-all duration-500 ease-in-out mobile-filters-container ${isFilterOpen ? 'max-h-screen opacity-100 filter-open' : 'max-h-0 opacity-0'}`}>
        <section className={`py-8 px-4 sm:px-6 lg:px-8 bg-gray-50 border-b border-gray-100 mobile-filters-wrapper ${isFilterOpen ? 'filter-open' : ''}`}>
          <div className="max-w-7xl mx-auto">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              {/* Type Filter */}
              <div>
                <h3 className="text-lg font-bold uppercase tracking-wider text-gray-700 mb-4">{t('projects.filter.type.label')}</h3>
              ...
              {/* City Filter */}
              <div>
                <h3 className="text-lg font-bold uppercase tracking-wider text-gray-700 mb-4">{t('projects.filter.location.label')}</h3>
                <div className="flex flex-wrap gap-3 mobile-filter-buttons">
                  {cities.map((city) => (
                    <button
                      key={city.key}
                ...
                {projectSections.map((section, sectionIndex) => (
                  <div key={section.key} className="mb-16" data-service-id={section.key}>
                    ...
                    <div className="text-center py-8">
                      <h2 className="text-4xl lg:text-5xl xl:text-6xl font-bold text-black uppercase tracking-wider">
                        {section.title}
                      </h2>
                    </div>
                        ...
                        <Link key={project.id} href={`/projects/${getProjectPageId(project.id)}`} className="group cursor-pointer block relative z-10 hover:z-20" id={`project-${project.id}`}>
                          <div className="flex items-center hover:bg-gray-50 transition-colors duration-300 flex-row">
                            ...
                            <div className="relative overflow-hidden bg-gray-100 aspect-square w-80 md:w-96 lg:w-[28rem] xl:w-[32rem] flex-shrink-0 group-hover:shadow-2xl transition-all duration-500 group-hover:scale-110 origin-left">
                              <img
                                src={project.image}
                                alt={translateProjectTitle(project.title)}
                            ...
                            <div className="flex-grow flex items-center justify-center">
                              <div className="space-y-4 text-right">
                                {/* Название */}
                                <h3 className="text-3xl lg:text-4xl xl:text-5xl font-bold text-black uppercase tracking-wider">
                                  {translateProjectTitle(project.title)}
                                </h3>

                                {/* Год */}
                                <div className="space-y-2">
                                  <span className="block text-3xl lg:text-4xl xl:text-5xl font-medium text-gray-600 uppercase tracking-wider">
                                    {project.year}
                                  </span>
                                </div>
                              </div>
                ...
                {projectSections.map((section, sectionIndex) => (
                  <div key={section.key} className="mb-8" data-service-id={section.key}>
                    ...
                    <div className="text-center py-6">
                      <h2 className="text-2xl lg:text-3xl font-bold text-black uppercase tracking-wider">
                        {section.title}
                      </h2>
                    </div>
                        ...
                        >
                          ...
                          <div className="absolute bottom-4 left-4 right-4 transform translate-y-0 opacity-100">
                            <div className="space-y-2">
                              ...
                              <div className="text-white font-medium uppercase tracking-wider text-sm opacity-80">
                                {project.year}
                              </div>
                            </div>
                          </div>
