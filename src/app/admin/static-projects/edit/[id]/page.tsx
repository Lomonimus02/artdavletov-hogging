...
export default function EditStaticProject({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    const loadProject = async () => {
      if (!isAdmin || isNaN(projectId)) return;

      try {
        // Сначала пытаемся загрузить из статических проектов
        let response = await fetch(`/api/admin/static-projects/${projectId}`, {
          headers: {
        ...
        if (!response.ok) {
          // Если не найден в статических, загружаем из публичного API
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(`/api/admin/static-projects/${projectId}`, {
        method: 'PUT',
        headers: {
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="py-6">
            ...
            <h1 className="text-3xl md:text-4xl font-black text-black uppercase tracking-wider">
              Редактировать статический проект
            </h1>
          </div>
        </div>
      </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="flex gap-4">
            <button
              type="submit"
              disabled={loading}
              className="bg-blue-600 text-white px-8 py-4 font-black uppercase tracking-wider text-xl hover:bg-blue-700 transition-colors duration-300 disabled:opacity-50 disabled:cursor-not-allowed"
            >
